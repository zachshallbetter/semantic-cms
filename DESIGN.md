...
`[P28]` **The upstream-debt ratchet.** Local patches against upstream are receipts, not forks: captured as diffs against the owning project, deployed if needed, and registered in `records/upstream-debts.jsonl` with an owner and a reversal trigger. Registered debts **age**: a standing warning that escalates, so a prose TODO cannot sit forever. Vendored upstream clones are replaced with pins; permanent forks are violations to remediate.
