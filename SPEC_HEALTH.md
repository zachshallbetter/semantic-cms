...
| SH-9 | **THIN — now actionable** | Import/migration | No pipeline from existing corpora (zachshallbetter.com archive, markdown trees) into Canon; the Admission plane (P1) is pending. | **rcp-001 confirmed 2026-08-28** — the zach-core archive is the first import (E8). Note: P1's Admission *plane* is undispositioned, but owner-authored content needs no admission gate: it lands as `declared` provenance under project.owner authority, which canonical v1 already supports. Admission becomes required when third-party content arrives. |
