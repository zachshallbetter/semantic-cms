...
The manifest asserts `preferImmutableRevisionOrDigest` and
`divergenceRequiresDeviation`, and ten resources carry a `revisionOrDigest`.
Nothing computed or checked them: `revisionOrDigest` appeared only in the
manifest, its schema, a generated projection, and one work file. The digests
were also *unreproducible* -- their prose said "aggregate sha256 of file
sha256s over the mount", which does not determine an answer. 48 reconstructions
against `fpb`, whose source is an immutable released package, matched none
(NR-scms-028).

So this file does two things a prose description cannot: it fixes one procedure,
and it fails when a source drifts.
...
Step 4 matches FPB's own `gen-context.py` aggregate. Steps 1 and 3 are what the
prose left open, and they are the whole reason the old digests cannot be
reproduced. A digest is only a claim if the procedure that produced it is
recorded, so a pin without `digestProcedure` is reported UNREPRODUCIBLE rather
than quietly recomputed into agreement.

## This is not a CI gate, and cannot be one

Every `canonicalSource` is an absolute path on the owner's machine
(`/Users/zachshallbetter/Projects/...`). A GitHub runner has none of them, so
the pins are unverifiable anywhere but here. That is a property of the pinning
scheme, not of this script (NR-scms-029).

    python3 scripts/verify-pins.py            # verify; nonzero on drift
    python3 scripts/verify-pins.py --update   # restamp under the procedure
...
MANIFEST = ROOT / "FORMAL_RESOURCE_MANIFEST.json"
PROCEDURE = "fpb-aggregate-v1"
...
def check(resource: dict) -> tuple[str, str]:
    ...
    # mount still holds what the manifest claims to import. `sss` ships EMPTY
    # schemas/ and conformance/ dirs (UD-13) while the code cites it as
    # structural authority, so hollowing is a real failure mode here, not a
    # hypothetical one.
    ...
    for r in manifest["resources"]:
        ...
        if "priorDigest" not in r:
            r["priorDigest"] = {
                ...
                "status": "procedure unrecorded; not reproducible as of 2026-08-30 (NR-scms-028)",
            }
        r["revisionOrDigest"] = f"sha256:{digest}"
        r["digestProcedure"] = PROCEDURE
        r["digestFileCount"] = count
        r["digestComputedAt"] = "2026-08-30"
        changed += 1
